function Get-FODUsers {
    <#
    .SYNOPSIS
        Get information about FOD users.
    .DESCRIPTION
        Get information about FOD users.
    .PARAMETER Filters
    ...
    .PARAMETER Paging
        If specified, and more data is available after loading limit of users,
        continue querying FOD until we have retrieved all the data available.
    .PARAMETER Limit
        Limit the number of users returned to this number.
        Maximum value is 50.
        Default is 50.
    .PARAMETER Raw
    ...
    .EXAMPLE
        # Get all of the users in the system through Paging
        Get-FODUsers -Paging
     .EXAMPLE
        # Get any users with "test" or "demo" in their username
        Get-FODUsers -Paging -Filters "userName:test|demo"
     .EXAMPLE
        # Get the userId of a specific user (for use in other operations)
        $user = Get-FODUsers -Filter "userName:testuser" -Fields "userId"
        Write-Host $user.userId
    .LINK
        https://api.ams.fortify.com/swagger/ui/index#!/Users/UsersV3_GetUsers
    .FUNCTIONALITY
        Fortify on Demand
    #>
    [CmdletBinding()]
    ...
    begin
    {
        ...
        Write-Verbose "Get-FODUsers Bound Parameters: $( $PSBoundParameters | Remove-SensitiveData | Out-String )"
        $Body = @{
            offset = 0
            limit = $Limit
        }
